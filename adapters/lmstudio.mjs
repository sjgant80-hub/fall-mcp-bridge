// LM Studio adapter · its OpenAI-compatible local server
...
export async function complete({ model, prompt, system, temperature, max_tokens, endpoint }) {
  ...
  if (!res.ok) throw new Error(`lmstudio ${res.status}: ${await res.text().catch(() => '')}`);
...
export async function listModels({ endpoint } = {}) {
  try {
    ...
    if (!res.ok) return [];
    const data = await res.json();
    return (data.data || []).map(m => ({ id: m.id, family: 'lmstudio' }));
  } catch {
    return [];
  }
}
